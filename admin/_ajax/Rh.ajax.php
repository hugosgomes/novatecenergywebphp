...
if ($PostData && $PostData['callback_action'] && $PostData['callback'] == $CallBack):
  ...
  switch ($Case):
    ...
    $variavelSelect = $Read->getResult();
      ...
      $Read->FullRead("SELECT [30_TipoDocumentacao].ID AS IDOC, [30_TipoDocumentacao].NOME AS NOMEDOC FROM [30_TipoCargosB]
      ...
      if ($idfuncionario) {
        foreach ($idfuncionario as $value) {
          ...
          $Read->FullRead("SELECT IdFuncionario AS id ,DataValidade AS data, Tipo, Status AS stat FROM [30_Documentacao] WHERE IdTipoDocumento = :IDDOC AND IdFuncionario = :IDFUNCI", "IDDOC={$value['IDOC']}&IDFUNCI={$PostData['pegarIdFUNC']}");
          ...
          if ($idtipo) {
            ...
            if ($variavelDataM > $dia_atual) {
              $variavelData = '<b><font color="green">'.$dataBr;
            } elseif ($variavelDataM <= $dia_atual) {
    ...
    //var_dump($PostData['idfuncionarioD']);
            ...
            if(isset($_FILES['documentosRH'])):
              ...
              if (!empty($d_Image)):
                ...
                foreach ($d_gbFiles as $d_UploadFile):
                        ...
                        if ($Upload->getResult()){
                          $d_gbCreate = array('IdFuncionario' => $variavel, 'Tipo' => $nomeAchilles, 'Caminho' => "$caminhoAchilles", 'Status' => 1, 'IdUsuario' => $_SESSION['userLogin']['ID'], 'DataValidade' => $datavalidade, 'IdTipoDocumento' => $IdTipoDocumento);
                ...
                //$variavelData = null;
                  if ($idfuncionario) {
                    foreach ($idfuncionario as $value) {
                      ...
                      $Read->FullRead("SELECT IdFuncionario AS id ,DataValidade AS data, Tipo, Status AS stat FROM [30_Documentacao] WHERE IdTipoDocumento = :IDDOC AND IdFuncionario = :IDFUNCI", "IDDOC={$value['IDOC']}&IDFUNCI={$PostData['funcionarioid']}");
                      $idtipo = $Read->getResult();
            ...
            if ($idfuncionario) {
              foreach ($idfuncionario as $value) {
                ...
                $Read->FullRead("SELECT IdFuncionario AS id ,DataValidade AS data, Tipo, Status AS stat FROM [30_Documentacao] WHERE IdTipoDocumento = :IDDOC AND IdFuncionario = :IDFUNCI", "IDDOC={$value['IDOC']}&IDFUNCI={$PostData['funcionarioDid']}");
                $idtipo = $Read->getResult();
